 ...
 * Spawns Claude Code or similar CLI tools to execute commands.
 */
...
export class ClaudeCodeAdapter {
  ...
  async execute(executionPackage, options = {}) {
    ...
    try {
      ...
      return {
        ...
        cost_usd: 0, // Local execution = no API cost
        tokens_used: 0,
      };
  ...
  _runCommand(command, cwd, onProgress, timeout = this.config.timeout) {
    return new Promise((resolve, reject) => {
      ...
      const child = spawn(this.config.shell, ['-Command', command], {
        cwd,
        env: {
          ...process.env,
          NODE_ENV: 'production',
   ...
   * @param {Object} executionPackage - Original execution package
   * @param {Object} options - Execution options
   * @returns {Promise<Object>} - Fallback result
   */
  async _executeFallback(fallback, executionPackage, options) {
    if (fallback.strategy === 'docker') {
      // Docker fallback implementation
      console.log(`Docker fallback not yet implemented for image: ${fallback.image}`);
      return {
        status: 'failed',
        error: 'Docker fallback not implemented',
      };
    }
    
    return {
      status: 'failed',
      error: `Unknown fallback strategy: ${fallback.strategy}`,
    };
  }
   ...
   * @returns {Promise<string>} - Path to work directory
   */
  async _createWorkDir(executionId) {
    const tmpDir = process.env.TMPDIR || process.env.TEMP || 'C:\\temp';
