...
export default function FileSummaryViewer({ projectId, projectFiles }) {
  ...
  const coverage = fileCount > 0 ? Math.round((summarizedCount / fileCount) * 100) : 0;
    ...
    <div style={S.card(false)}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span style={S.label(C.blue2)}>
          📝 File Summaries 
          <span style={{ ...S.badge(coverage >= 80 ? C.green : coverage >= 50 ? C.amber : C.dim), marginLeft: 8 }}>
            {coverage}% covered
          </span>
        </span>
      ...
      {!loading && summaries.length === 0 && (
        <div style={{ fontSize: 10, color: C.muted, padding: "12px 0", lineHeight: 1.6 }}>
          No summaries yet. Summaries are generated automatically when you save files (markdown, code, JSON).
          <br />
          <span style={{ color: C.dim }}>L0 = 1-sentence abstract | L1 = Structured overview</span>
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 400, overflow: "auto" }}>
          {summaries.map((s) => (
            <div 
              ...
              onClick={() => setExpanded(expanded === s.file_path ? null : s.file_path)}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <span style={{ fontSize: 8, color: C.dim, whiteSpace: "nowrap" }}>
              ...
              {expanded === s.file_path && s.l1_overview && (
                <div style={{ 
                  ...
                  <div style={{ ...S.label(C.blue), marginBottom: 4 }}>L1 Overview</div>
                  {s.l1_overview}
                </div>
              )}
      ...
      <div style={{ marginTop: 10, fontSize: 8, color: C.dim }}>
        Click any file to expand L1 overview. Summaries update automatically on file save.
      </div>
    </div>
  );
}
