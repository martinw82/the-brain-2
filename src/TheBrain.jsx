...
const STANDARD_FOLDERS = [
  ...
  {id:"tools",             icon:"🔧",label:"Tools",             desc:"Scripts, automation, utilities"},
];
...
`;
  files["tools/word-count.js"] = `// Count words across all markdown files in project
...
const GitHubIntegration=({projects,isMobile})=>{
  ...
  const [form,setForm]=useState({owner:"",repo:"",token:"",branch:"main"});
  ...
  const handleDisconnect=async()=>{
    if(!confirm("Disconnect GitHub? Your files will remain in The Brain, but sync will stop."))return;
    try{
  ...
  return(
    <div>
      ...
      <div style={{marginBottom:16}}>
        ...
        <select style={S.sel} value={selectedProject} onChange={e=>setSelectedProject(e.target.value)}>
          {projects.map(p=><option key={p.id} value={p.id}>{p.emoji} {p.name}</option>)}
        </select>
      </div>
      
      {/* Help Box */}
      <div style={{background:`${C.blue}08`,border:`1px solid ${C.blue}30`,borderRadius:8,padding:12,marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{fontSize:11,fontWeight:600,color:C.blue}}>💡 What is GitHub Integration?</span>
          <button style={{...S.btn("ghost"),fontSize:9,padding:"4px 8px"}} onClick={()=>setShowHelp(!showHelp)}>{showHelp?"Hide":"Learn more"}</button>
        </div>
        {showHelp&&(
          <div style={{fontSize:10,color:C.text,lineHeight:1.6}}>
            <p style={{margin:"0 0 8px"}}>
              Connect your Brain project to a <strong>GitHub repository</strong> for backup and version control.
            </p>
            ...
            <p style={{margin:"8px 0 0",color:C.muted}}>
              💡 <strong>Note:</strong> This syncs your <em>planning files</em> (PROJECT_OVERVIEW.md, specs, etc.). 
              Your actual code repos are separate—link to them in your project overview.
            </p>
          </div>
        )}
      </div>
      ...
      {/* Error state */}
      {error&&(
        <div style={{background:`${C.red}10`,border:`1px solid ${C.red}40`,borderRadius:8,padding:12,marginBottom:16}}>
      ...
      {!loading&&!githubData?.connected&&(
        <div style={S.card(false)}>
          <div style={{textAlign:"center",padding:"20px 0"}}>
            ...
            <div style={{fontSize:14,fontWeight:600,color:"#f1f5f9",marginBottom:8}}>GitHub Not Connected</div>
            <div style={{fontSize:10,color:C.muted,marginBottom:16}}>
              Connect this project to a GitHub repository to enable sync.
            </div>
            <button style={S.btn("primary")} onClick={()=>setShowConnect(true)}>Connect GitHub</button>
          </div>
        </div>
      )}
      ...
      {!loading&&githubData?.connected&&githubData?.repo&&(
        <div>
          ...
          <div style={S.card(true)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div style={{display:"flex",gap:12}}>
                ...
                <div>
                  <div style={{fontSize:14,fontWeight:700,color:"#f1f5f9"}}>{githubData.repo.name}</div>
                  <div style={{fontSize:10,color:C.muted}}>{githubData.repo.full_name}</div>
                  {githubData.repo.description&&(
                    <div style={{fontSize:9,color:C.dim,marginTop:4}}>{githubData.repo.description}</div>
                  )}
                </div>
              </div>
            ...
            <div style={{display:"flex",gap:16,marginBottom:16}}>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:16,fontWeight:700,color:C.text}}>{githubData.repo.stargazers_count}</div>
                <div style={{fontSize:8,color:C.dim,textTransform:"uppercase"}}>Stars</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:16,fontWeight:700,color:C.text}}>{githubData.repo.forks_count}</div>
                <div style={{fontSize:8,color:C.dim,textTransform:"uppercase"}}>Forks</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:16,fontWeight:700,color:C.text}}>{githubData.repo.open_issues_count}</div>
                <div style={{fontSize:8,color:C.dim,textTransform:"uppercase"}}>Issues</div>
              </div>
            </div>
            
            <div style={{display:"flex",gap:8}}>
              <button style={S.btn("ghost")} onClick={()=>window.open(githubData.repo.html_url,'_blank')}>
                🌐 Open Repo
              </button>
              <button style={{...S.btn("ghost"),borderColor:C.red,color:C.red}} onClick={handleDisconnect}>
                Disconnect
              </button>
            </div>
          </div>
          ...
          <div style={S.card(false)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <span style={S.label()}>Recent Commits ({githubData.branch})</span>
              <span style={{fontSize:9,color:C.dim}}>
                Last sync: {githubData.last_sync_at ? new Date(githubData.last_sync_at).toLocaleString() : 'Never'}
              </span>
            </div>
            ...
            {githubData.commits?.length===0?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {githubData.commits?.map((commit,i)=> (
                  <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"8px",background:C.bg,borderRadius:6}}>
                    <div style={{fontSize:10,fontWeight:700,color:C.blue,minWidth:50}}>{commit.sha}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:10,color:C.text}}>{commit.message}</div>
                      <div style={{fontSize:8,color:C.dim,marginTop:2}}>
                        {commit.author} • {new Date(commit.date).toLocaleDateString()}
                      </div>
                    </div>
                    <a href={commit.html_url} target="_blank" rel="noreferrer" style={{fontSize:9,color:C.blue}}>View →</a>
                  </div>
                ))}
              </div>
            )}
      ...
      {!loading&&githubData?.connected&&!githubData?.repo&&(
        <div style={S.card(false)}>
          <div style={{background:`${C.amber}10`,border:`1px solid ${C.amber}40`,borderRadius:8,padding:12,marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:600,color:C.amber,marginBottom:4}}>⚠ Connection Issue</div>
            <div style={{fontSize:9,color:C.text}}>
              {githubData.error || "Could not fetch repository data. Your token may have expired or the repo was deleted."}
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button style={S.btn("primary")} onClick={()=>setShowConnect(true)}>Reconnect</button>
            <button style={S.btn("ghost")} onClick={handleDisconnect}>Disconnect</button>
          </div>
        </div>
      )}
      ...
      {/* Connect Modal */}
      {showConnect&&(
        <Modal title="Connect to GitHub" onClose={()=>{setShowConnect(false);setError(null);}} width={420}>
          <div style={{marginBottom:16}}>
            <div style={{background:`${C.blue}08`,border:`1px solid ${C.blue}30`,borderRadius:8,padding:12,marginBottom:16}}>
              <div style={{fontSize:10,color:C.text,lineHeight:1.6}}>
                ...
                <a href="https://github.com/settings/tokens" target="_blank" rel="noreferrer" style={{fontSize:9,color:C.blue}}>
            ...
            {error&&(
              <div style={{background:`${C.red}10`,border:`1px solid ${C.red}40`,borderRadius:6,padding:10,marginBottom:12}}>
                <div style={{fontSize:10,color:C.red}}>⚠ {error}</div>
              </div>
            )}
            ...
            <div style={{marginBottom:12}}>
              ...
              />
              <div style={{fontSize:8,color:C.dim,marginTop:4}}>Your GitHub username or organization</div>
            </div>
            ...
            <div style={{marginBottom:12}}>
              ...
              />
              <div style={{fontSize:8,color:C.dim,marginTop:4}}>The repo where your Brain files will sync</div>
            </div>
            ...
            <div style={{marginBottom:12}}>
              ...
              <input 
                ...
                placeholder="main"
              />
              <div style={{fontSize:8,color:C.dim,marginTop:4}}>Default branch to sync (usually 'main')</div>
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:8,color:C.dim,marginTop:4}}>
                Token with <strong>repo</strong> scope. Never shared or displayed again.
              </div>
            </div>
            
            <div style={{display:"flex",gap:8}}>
              <button style={S.btn("primary")} onClick={handleConnect} disabled={saving}>
                {saving?"Connecting...":"Connect to GitHub"}
              </button>
              <button style={S.btn("ghost")} onClick={()=>setShowConnect(false)}>Cancel</button>
            </div>
          </div>
        </Modal>
      )}
...
const ScriptRunner=({projectId,projectFiles})=>{
  ...
  useEffect(()=>{
    const foundScripts=[];
    for(const [path,content] of Object.entries(projectFiles||{})){
      if(path.startsWith('tools/')&&(path.endsWith('.js')||path.endsWith('.py'))){
        const name=path.split('/').pop().replace(/\\.[^.]+$/,'');
        // Try to extract metadata from script
        let meta={name,description:'',language:path.endsWith('.py')?'python':'javascript'};
  ...
  return<div style={{border:`1px solid ${C.border}`,borderRadius:8,padding:12,marginBottom:16,background:C.surface}}>
    ...
    {expanded&&(
      <div style={{marginTop:12}}>
        {/* Predefined Scripts */}
        <div style={{marginBottom:12}}>
          <div style={{fontSize:9,color:C.dim,marginBottom:6}}>Quick Scripts:</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={()=>runPredefined('wordcount')} style={{...S.btn("ghost"),fontSize:9,padding:"4px 10px"}}>📝 Word Count</button>
            <button onClick={()=>runPredefined('todos')} style={{...S.btn("ghost"),fontSize:9,padding:"4px 10px"}}>✓ List TODOs</button>
            <button onClick={()=>runPredefined('export')} style={{...S.btn("ghost"),fontSize:9,padding:"4px 10px"}}>📊 Stats</button>
          </div>
        </div>
        ...
        {scripts.length>0&&(
          <div style={{marginBottom:12}}>
            <div style={{fontSize:9,color:C.dim,marginBottom:6}}>Custom Scripts (from tools/):</div>
            <select 
            ...
            {selectedScript&&selectedScript.meta.description&&(
              <div style={{fontSize:9,color:C.dim,marginTop:4}}>{selectedScript.meta.description}</div>
            )}
          </div>
        )}
        ...
        <button 
          ...
          style={{...S.btn("primary"),fontSize:9,padding:"4px 12px",opacity:(!selectedScript||running)?0.5:1}}
        >
