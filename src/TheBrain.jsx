...
import { projects as projectsApi, staging as stagingApi, ideas as ideasApi, sessions as sessionsApi, comments as commentsApi, search as searchApi } from "./api.js";
...
const renderMd = (md = "") => {
  if (!md) return "";
  const html = md.replace(/^### (.*)$/gm, "<h3 style='color:#e2e8f0;font-size:13px;margin:12px 0 6px'>$1</h3>")
    .replace(/^## (.*)$/gm, "<h2 style='color:#f1f5f9;font-size:15px;margin:16px 0 8px;border-bottom:1px solid #0f1e3a;padding-bottom:4px'>$1</h2>")
    .replace(/^# (.*)$/gm, "<h1 style='color:#f1f5f9;font-size:18px;margin:0 0 16px;font-weight:700'>$1</h1>")
    .replace(/\*\*(.*?)\*\*/g, "<strong style='color:#e2e8f0'>$1</strong>")
    .replace(/`([^`]+)`/g, "<code style='background:#0d1424;border:1px solid #1e293b;padding:1px 5px;border-radius:3px;font-size:11px;color:#10b981'>$1</code>")
    .replace(/^- \[x\] (.*)$/gm, "<div style='display:flex;gap:6px;padding:2px 0'><span style='color:#10b981'>✅</span><span>$1</span></div>")
    .replace(/^- \[ \] (.*)$/gm, "<div style='display:flex;gap:6px;padding:2px 0'><span style='color:#334155'>⬜</span><span style='color:#94a3b8'>$1</span></div>")
    .replace(/^- (.*)$/gm, "<div style='display:flex;gap:6px;padding:2px 0'><span style='color:#1a4fd6'>·</span><span>$1</span></div>")
    .replace(/^\| (.*) \|$/gm, (row) => {
    ...
    .replace(/^> (.*)$/gm, "<blockquote style='border-left:3px solid #1a4fd6;margin:8px 0;padding:6px 12px;color:#94a3b8;font-style:italic'>$1</blockquote>");
  return html.replace(/\n/g, "<br />");
};
...
export default function TheBrain({ user, initialProjects = [], initialStaging = [], initialIdeas = [], onLogout }) {
  ...
  const openHub = useCallback(async (id, file = "PROJECT_OVERVIEW.md") => {
    ...
    // This check is the core of the lazy-loading fix
    if (project && !project.files) {
      setLoadingFiles(true);
      try {
        const { project: loadedProject } = await projectsApi.get(id);
        const camelCasedFiles = Object.entries(loadedProject.files).reduce((acc, [key, value]) => {
        ...
        setProjects(prev => prev.map(p =>
          p.id === id ? { ...p, ...loadedProject, files: camelCasedFiles || {}, activeFile: file } : p
        ));
      } catch (e) {
...
`;
    ...
    try {
      ...
      const s = { project_id: projId, name: "Bootstrap complete — run Strategy Agent next", tag: "DRAFT_", status: "in-review", notes: "Copy STRATEGY_PROMPT.md → paste into Claude → save output as STRATEGY_OUTPUT.md", added: new Date().toISOString().slice(0, 7) };
      const res = await stagingApi.create(s);
      setStaging(prev => [...prev, { ...s, id: res.id || `tmp-${Date.now()}`, project: projId }]);
  ...
  const handleDrop = useCallback((e, projId) => {
    ...
    Array.from(e.dataTransfer.files).forEach(file => {
      ...
      if (file.type.startsWith("text") || ["md", "json", "js", "ts", "py", "sol", "txt", "css", "html"].some(ext => file.name.endsWith("." + ext))) reader.readAsText(file);
  ...
  const buildBrief = (skillId, projId) => {
    const sk = SKILLS[skillId]; const proj = projects.find(p => p.id === projId); if (!sk || !proj) return ""; return `# ${sk.icon} ${sk.label} Briefing — ${proj.emoji} ${proj.name}
...
\`\`\``;
  ...
  const askAI = async (prompt) => {
    ...
    try {
      const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": "YOUR_ANTHROPIC_API_KEY" }, body: JSON.stringify({ model: "claude-3-haiku-20240307", max_tokens: 1000, system: sys, messages: [{ role: "user", content: prompt }] }) });
