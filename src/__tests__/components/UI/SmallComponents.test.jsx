...
describe('SmallComponents', () => {
  ...
  describe('Dots', () => {
    ...
    it('should highlight correct number of dots', () => {
      ...
      expect(dots[0]).toHaveStyle({ background: expect.stringContaining('3b82f6') });
      expect(dots[1]).toHaveStyle({ background: expect.stringContaining('3b82f6') });
    });
  ...
  describe('HealthBar', () => {
    it('should show green for high health', () => {
      ...
      expect(screen.getByText('80')).toHaveStyle({ color: expect.stringContaining('10b981') });
    });

    it('should show amber for medium health', () => {
      render(<HealthBar score={50} />);
      
      expect(screen.getByText('50')).toHaveStyle({ color: expect.stringContaining('f59e0b') });
    });

    it('should show red for low health', () => {
      render(<HealthBar score={30} />);
      
      expect(screen.getByText('30')).toHaveStyle({ color: expect.stringContaining('ef4444') });
    });
