...
async function selectAgentForTask(task, userId) {
  ...
  const scored = await Promise.all(permitted.map(async a => {
...
async function onTaskComplete(taskId) {
  ...
  await workflowInstances.update(instance.id, {
    ...
    [`step_results.step_${task.workflow_step_id}.completed_at`]: new Date().toISOString(),
    $append: {
...
### Phase 5.6: Agent Task Execution
**Goal:** Agents actually DO work (function calling)
**Depends on:** Both 5.3 and 5.5

- [ ] Define agent actions (read_file, write_file, etc.)
- [ ] Implement function calling in AI proxy
- [ ] Sandbox file operations
- [ ] Preview mode vs Auto mode
- [ ] Execution tracing
